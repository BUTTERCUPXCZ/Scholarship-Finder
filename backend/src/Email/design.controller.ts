...
// Build a nice, email-client-friendly verification HTML with inline styles and a text fallback
export const buildVerificationEmail = (fullname: string, verifyUrl: string) => {
...
// Build a styled password reset OTP email (HTML + plain text)
export const buildPasswordResetEmail = (fullname: string, otp: string, minutes = 10) => {
    const safeName = fullname || 'there';
